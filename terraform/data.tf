...
# and public SSH keys from the repo's .ssh directory.
...
locals {
  ...
  # host. Lives in Infisical so HCP Terraform can read it at runtime
  # without the key ever touching disk in the cloud runner.
  terraform_bot_private_key = data.infisical_secrets.proxmox.secrets["TERRAFORM_BOT_PRIVATE_KEY"].value

  # Public SSH keys live in .ssh/ in the repo. Reading them with
  # file() means the keys exist in only one place — the repo —
